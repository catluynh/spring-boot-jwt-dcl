package com.springbootjwtdcl.filter;
...
import com.springbootjwtdcl.service.TokenService;
import com.springbootjwtdcl.util.JwtUtil;
...
public class JwtRequestFilter extends OncePerRequestFilter{
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (null != user && null != token && token.getTokenExpDate().after(new Date())) {
			...
			user.getAuthorities().forEach(p -> authorities.add(new SimpleGrantedAuthority(p+"")));
